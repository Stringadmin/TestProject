...
exports.processComfyUIRequest = async (prompt, designImage, workflowName, workflowPath) => {
    try {
        ...
        // 添加随机种子，确保每次提交的工作流都是唯一的，强制ComfyUI重新生成图像
        const timestamp = Date.now();
        const randomId = Math.random().toString(36).substr(2, 9);
        
        // 为KSampler节点设置随机种子，这是确保生成不同图像的最直接方法
        for (const nodeId in workflow) {
            const node = workflow[nodeId];
            // 寻找KSampler或类似的采样器节点
            if (node && typeof node === 'object' && /Sampler/i.test(node.class_type) && node.inputs) {
                // 设置随机种子，确保每次生成不同的图像
                node.inputs.seed = timestamp; // 使用时间戳作为种子
                console.log(`[ComfyUI Request] 已为${node.class_type}节点设置随机种子: ${timestamp}`);
                break; // 找到第一个采样器节点就可以了
            }
        }
        ...
        workflow.metadata.request_id = `${timestamp}_${randomId}`;
        workflow.metadata.timestamp = timestamp;
...
exports.submitComfyUIPrompt = async (prompt, designImage, workflowName, workflowPath) => {
    ...
    for (const nodeId in workflow) {
        ...
        if (node && typeof node === 'object' && /Sampler/i.test(node.class_type) && node.inputs) {
            // 设置随机种子，确保每次生成不同的图像
            node.inputs.seed = timestamp; // 使用时间戳作为种子
            console.log(`[submitComfyUIPrompt] 已为${node.class_type}节点设置随机种子: ${timestamp}`);
            break; // 找到第一个采样器节点就可以了
        }
    }
    ...
    // 在请求负载中也添加时间戳
    const payload = { prompt: workflow, client_id: 'archvisualizer-web', request_timestamp: timestamp };
            ...
            for (const nodeId in output) {
                ...
                if (output[nodeId].images && output[nodeId].images.length > 0) {
                    for (const img of output[nodeId].images) {
                        ...
                        // 使用正确的图像代理路径而不是直接访问ComfyUI
                        const subfolderValue = img.subfolder || '';
                        const typeValue = img.type || 'output';
                        const proxyUrl = `/comfyui/image-proxy?filename=${encodeURIComponent(img.filename)}&subfolder=${encodeURIComponent(subfolderValue)}&type=${encodeURIComponent(typeValue)}`;
...
exports.setupImageProxy = (app) => {
    ...
    app.get('/comfyui/image-proxy', async (req, res) => {
        ...
        try {
            ...
            // 实现带重试的图像请求函数
            const fetchImageWithRetry = async (url, retries = 3, delay = 2000) => {
                for (let attempt = 1; attempt <= retries; attempt++) {
                    try {
                        console.log(`[${new Date().toISOString()}] 发送请求到ComfyUI... (尝试 ${attempt}/${retries})`);
                        ...
                        console.log(`[${new Date().toISOString()}] 尝试 ${attempt} 失败:`, error.message);
                        // 如果不是最后一次尝试，等待后重试
                        if (attempt < retries) {
                            console.log(`[${new Date().toISOString()}] ${delay}ms 后重试...`);
            ...
            // 使用带重试的函数获取图像
            const response = await fetchImageWithRetry(originalImageUrl);
            ...
            res.set('Access-Control-Allow-Origin', '*'); // 允许跨域请求
            ...
            if (error.code === 'ECONNABORTED') {
                errorMessage = '图像加载超时，请检查网络连接';
                statusCode = 408; // Request Timeout
            } else if (error.response) {
                errorMessage = `获取图像失败: ${error.response.status} ${error.response.statusText}`;
